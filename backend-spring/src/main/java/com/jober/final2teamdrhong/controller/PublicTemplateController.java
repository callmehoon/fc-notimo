...
public class PublicTemplateController {
     ...
     *                  - page: 페이지 번호 (0부터 시작)
     *                  - size: 페이지 크기
     *                  - sort: 정렬 필드 (허용값: createdAt, shareCount, viewCount, publicTemplateTitle)
     *                  - direction: 정렬 방향 (허용값: ASC, DESC)
     *                  - search.keyword: 검색 키워드 (제목/내용 대상)
     *                  - search.searchTarget: 검색 대상 필드 (허용값: TITLE, CONTENT, ALL, 기본값: ALL)
    ...
    @Operation(
            ...
            description = "삭제되지 않은 공용 템플릿 목록을 페이징하여 조회합니다." +
                    ...
                    "검색 키워드(keyword)와 검색 대상(searchTarget)을 포함할 수 있습니다. " +
                    "쿼리 파라미터: " +
                    "page(페이지번호, 기본값:0), size(페이지크기, 기본값:10), " +
                    "sort(정렬필드, 기본값:createdAt), direction(정렬방향, 기본값:DESC), " +
                    "search.keyword(검색어), search.searchTarget(검색 대상 필드, 기본값:ALL, 허용값: TITLE, CONTENT, ALL)",
    ...
    public Page<PublicTemplateResponse> getPublicTemplates(
        ...
        return publicTemplateService.getTemplates(request);
    }
