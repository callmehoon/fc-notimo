...
public class IndividualTemplateController {
    ...
    @Operation(summary = "빈 템플릿 생성", description = "Workspace ID를 기반으로 빈 템플릿을 생성합니다.")
    @ApiResponses({
    ...
    public ResponseEntity<IndividualTemplateResponse> createEmptyTemplate(
        ...
        return ResponseEntity.ok(individualTemplateService.createTemplate(workspaceId));
    }

    // 빈 템플릿 생성 (비동기)
    @PostMapping("/templates/{workspaceId}/async")
    @Operation(summary = "빈 템플릿 생성(비동기 @Async)", description = "가상 스레드 확인용")
